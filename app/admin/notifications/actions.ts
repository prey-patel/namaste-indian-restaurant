...
/**
 * Sends a test push notification to all active devices registered by the logged-in user.
 */
export async function sendTestPushAction() {
  try {
    ...
    if (error || !subs || subs.length === 0) {
      return {
        success: false,
        error: 'No active push notification registrations found for your account on the server.'
      };
    }
    ...
    for (const sub of subs) {
      try {
        await sendWebPush(
          ...
          'Test Notification 🔔',
          'Your push notification pipeline is working perfectly! 🎉',
          { clickPath: '/admin/orders' }
        );
        sentCount++;
      } catch (err: any) {
        console.error(`Failed to send test push to sub ID ${sub.id}:`, err);
      }
    }

    if (sentCount === 0) {
      return { success: false, error: 'Failed to send test push to registered devices.' };
    }
